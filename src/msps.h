...
typedef struct image {
  ...
  struct image_list *img_data_node;  /* backpointer for O(1) remove */
} IMAGE_t;

typedef struct image_list {
  IMAGE_t *to_image;
  struct image_list *prev;  /* doubly-linked for O(1) removal */
...
int scan_msp(MSP_t *m, char *line) {
    ...
    if (sscanf(line, "%ld %f %ld %ld %s %ld %ld %s \n", &(m->score), &(m->iden), &(m->query.frag.lb), &(m->query.frag.rb), qname, &(m->sbjct.frag.lb), &(m->sbjct.frag.rb), sname) != 8) {
      ...
      m->query.to_msp = m;
      m->sbjct.to_msp = m;
      m->query.img_data_node = NULL;
      m->sbjct.img_data_node = NULL;

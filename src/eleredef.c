...
typedef struct img_node {
  ...
  IMAGE_t *to_image;
  struct img_node *sib;
  struct img_node *sib_tail;  /* tail pointer for O(1) sibling append */
...
void adjust_img_tree(ELE_INFO_t *);
...
int main (int argc, char *argv[]) {
  ...
  if (new_stat) {
	...
	if (line[i] == ' ') break;
      ...
      stat = line[i+1];
      fu = atoi(&line[i+3]);
      if (ei > ele_array_size) grow_ele_array(ei);
...
ELE_INFO_t *new_element() {
  ...
  if (ele_ct > ele_array_size) grow_ele_array(ele_ct);
...
void remove_ele(ELE_INFO_t *ele_info) {
    ...
    unlink(fn);  /* O(1) syscall instead of fork+exec+shell */
}
...
void ele_redef(ELE_INFO_t *ele_info, IMAGE_t **img_ptr) {
    ...
    if (!cur_ele->to_img_data) {
      listify(cur_ele->to_img_tree, &cur_ele->to_img_data);
      listify_set_prev(cur_ele->to_img_data);
    }
...
IMG_DATA_t *img_data_sort(IMG_DATA_t *img_data, int ct) {
  ...
  for (i=0; i<ct; i++) {
    ...
    (*(img_data_ptr+i+1))->prev = *(img_data_ptr+i);
  }
...
void put_image(IMG_DATA_t **idp, IMAGE_t *i) {
  ...
  img_data_tmp = (IMG_DATA_t *)pool_alloc(&img_data_pool, sizeof(IMG_DATA_t));
  img_data_tmp->to_image = i;
  img_data_tmp->prev = NULL;
  img_data_tmp->next = *idp;
  if (*idp) (*idp)->prev = img_data_tmp;
  *idp = img_data_tmp;
  i->img_data_node = img_data_tmp;
}
...
void remove_image(IMAGE_t *i) {
    IMG_DATA_t *node = i->img_data_node;

    if (!node) return;  /* no img_data_node link */

    /* O(1) doubly-linked list removal */
    if (node->prev) {
      node->prev->next = node->next;
    } else {
      i->ele_info->ele->to_img_data = node->next;
    }
    if (node->next) {
      node->next->prev = node->prev;
    }
    i->img_data_node = NULL;
    pool_free(&img_data_pool, node);
}
...
void edges_and_cps(ELE_INFO_t *ele_info, IMAGE_t **img_ptr) { /* the core function in this part, calls everything below */
  ...
  if (ele_info->ele->to_img_tree) adjust_img_tree(ele_info);
...
void adjust_img_tree(ELE_INFO_t *ele_info) {
  IMAGE_t **img_array;
  int ct, ct1;

  ct = ele_info->ele->img_no;
  if (ct <= 0) return;
  img_array = (IMAGE_t **) malloc(ct * sizeof(IMAGE_t *));
  ct1 = charge_img_array(img_array, ele_info->ele->to_img_tree, 0);
  if (ct1 != ct) {
    err_no ++;
    fprintf(log_file, "error:  trouble charging the img array in ele %d: %d charged, %d expected\n", ele_info->index, ct1, ct);
    exit(4);
  }
  img_tree_free_nodes(&ele_info->ele->to_img_tree);
  build_img_tree(&ele_info->ele->to_img_tree, img_array, 0, ct-1);
  free(img_array);
}
...
int consis_tree_build(IMG_NODE_t *rt, IMAGE_t *im, int prequal) {
  ...
  if (prequal || consis(rt->to_image, im, CUTOFF2)) {
    ...
    if (!sum) {
      ...
      node->sib_tail = NULL;
      node->children = NULL;
      /* O(1) append via sib_tail pointer instead of O(K) node_entry() */
      if (rt->children) {
        rt->sib_tail->sib = node;
      } else {
        rt->children = node;
      }
      rt->sib_tail = node;

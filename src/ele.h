...
static inline void *pool_alloc(POOL_t *pool, size_t item_size) {
  void *item;
  if (pool->free_list) {
    item = pool->free_list;
  ...
  if (pool->cur_ptr == NULL || pool->cur_ptr >= pool->cur_end) {
    size_t alloc_size = POOL_BLOCK_ITEMS * item_size;
    pool->cur_ptr = (char *)malloc(alloc_size);
    pool->cur_end = pool->cur_ptr + alloc_size;
  }
  item = pool->cur_ptr;
  pool->cur_ptr += item_size;
  return item;
}

static inline void pool_free(POOL_t *pool, void *item) {
  *(void **)item = pool->free_list;
  pool->free_list = item;
}

POOL_t msp_pool, edge_pool, img_tree_pool, edge_tree_pool, img_data_pool;
...
void listify_set_prev(IMG_DATA_t *);
...
void img_tree_free_nodes(IMG_TREE_t **);
int charge_img_array(IMAGE_t **, IMG_TREE_t *, int);
...
MSP_t *MSP_malloc() {
  MSP_t *msp_tmp = (MSP_t *) pool_alloc(&msp_pool, sizeof(MSP_t));
  memset(msp_tmp, 0, sizeof(MSP_t));
  msp_in_mem ++;
  msp_ct ++;
...
EDGE_t *EDGE_malloc() {
  EDGE_t *edge_tmp = (EDGE_t *) pool_alloc(&edge_pool, sizeof(EDGE_t));
...
ELEMENT_t *ele_read_in(ELE_INFO_t *ele_info, int stage) {
  ...
  fp = fopen(fn, "r");
  if (fp) setvbuf(fp, NULL, _IOFBF, 65536);  /* 64K read buffer */
  files_read ++;
...
void grow_ele_array(int needed_size) {
  int new_size, i;
  if (needed_size <= ele_array_size) return;
  new_size = ele_array_size * 2;
  if (new_size < needed_size) new_size = needed_size;
  all_ele = (ELE_INFO_t **) realloc(all_ele, new_size * sizeof(ELE_INFO_t *));
  for (i = ele_array_size; i < new_size; i++) {
    *(all_ele+i) = ele_info_init(i+1);
  }
  ele_array_size = new_size;
}

ELE_INFO_t *get_ele_info(int index) {
  if (index > ele_array_size) grow_ele_array(index);
  return *(all_ele+index-1);
}
...
void ele_write_out(ELE_INFO_t *ele_info, int stage) {
  ...
  fp = fopen(fn, "w");
  setvbuf(fp, NULL, _IOFBF, 65536);  /* 64K write buffer */
...
void write_out_msps(FILE *fp, IMG_TREE_t *rt) {
  ...
  if (rt->to_image->to_msp->query.ele_info->index != rt->to_image->to_msp->sbjct.ele_info->index || rt->to_image == &rt->to_image->to_msp->sbjct) { /* trick to report self MSPs only once */
    ...
    fprintf(fp, "%ld %3.1f %d ", rt->to_image->to_msp->score, rt->to_image->to_msp->iden, rt->to_image->to_msp->direction);
    fprintf(fp, "%d %s %ld %ld ", rt->to_image->to_msp->query.ele_info->index, rt->to_image->to_msp->query.frag.seq_name, rt->to_image->to_msp->query.frag.lb, rt->to_image->to_msp->query.frag.rb);
...
void build_img_tree(IMG_TREE_t **rt_ptr, IMAGE_t **img_array, int ori, int end) {
  int mid = (ori+end)/2;
  IMG_TREE_t *new = (IMG_TREE_t *) pool_alloc(&img_tree_pool, sizeof(IMG_TREE_t));
...
void insert_image(IMG_TREE_t **rt_ptr, IMAGE_t *i) {
  ...
  new = (IMG_TREE_t *) pool_alloc(&img_tree_pool, sizeof(IMG_TREE_t));
  new->to_image = i;
  new->p = NULL;
  new->l = NULL;
  new->r = NULL;
...
IMG_DATA_t **listify(IMG_TREE_t *rt, IMG_DATA_t **tail_ptr) {
  ...
  new = (IMG_DATA_t *) pool_alloc(&img_data_pool, sizeof(IMG_DATA_t));
  new->to_image = rt->to_image;
  new->next = NULL;
  new->prev = NULL;
  *tail_ptr = new;
  rt->to_image->img_data_node = new;
  tail_ptr = &new->next;
...
/* Set prev pointers after listify builds the list */
void listify_set_prev(IMG_DATA_t *head) {
  IMG_DATA_t *prev = NULL, *cur = head;
  while (cur) {
    cur->prev = prev;
    prev = cur;
    cur = cur->next;
...
void build_edge_tree(EDGE_TREE_t **rt_ptr, EDGE_t **edge_array, int ori, int end) {
  int mid = (ori+end)/2;
  EDGE_TREE_t *new = (EDGE_TREE_t *) pool_alloc(&edge_tree_pool, sizeof(EDGE_TREE_t));
...
void insert_edge(EDGE_TREE_t **rt_ptr, EDGE_t *ed) {
  ...
  new = (EDGE_TREE_t *) pool_alloc(&edge_tree_pool, sizeof(EDGE_TREE_t));
  new->to_edge = ed;
  new->p = NULL;
  new->l = NULL;
  new->r = NULL;
...
/* Free tree nodes only, without decrementing img_no.
   Used by adjust_img_tree to rebuild a balanced tree. */
void img_tree_free_nodes(IMG_TREE_t **rt_p) {
  if ((*rt_p)->l) img_tree_free_nodes(&(*rt_p)->l);
  if ((*rt_p)->r) img_tree_free_nodes(&(*rt_p)->r);
...
int charge_img_array(IMAGE_t **img_array, IMG_TREE_t *rt, int pos) {
  if (rt->l) pos = charge_img_array(img_array, rt->l, pos);

  *(img_array+pos) = rt->to_image;
  pos ++;

  if (rt->r) pos = charge_img_array(img_array, rt->r, pos);
